...
const QuoteEditModal: React.FC<QuoteEditModalProps> = ({ quote, onClose, onUpdate }) => {
  const { user } = useAuth();
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({
    status: quote.status,
    estimatedCost: quote.estimatedCost || 0,
    validUntil: quote.validUntil && typeof quote.validUntil.toDate === 'function'
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-xl">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Edit Quote: {quote.quoteNumber}</h2>
          <button
            onClick={onClose}
